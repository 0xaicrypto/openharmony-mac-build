...
#ifdef __APPLE__
/* Apple's <sys/types.h> typedefs uuid_t as unsigned char[16], which clashes
 * with the kernel's struct uuid_t used by mod_devicetable.h. Suppress the
 * system one by renaming everything to ohos_uuid_t. */
#define _UUID_T
#define uuid_t ohos_uuid_t
typedef struct { unsigned char b[16]; } ohos_uuid_t;
#endif
