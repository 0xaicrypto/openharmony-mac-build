...
/* debug ring buffer of recent __cxa_atexit registrations, dumped by SIGSEGV handler */
void *dbg_atexit_log[64][3];
int dbg_atexit_idx;
	...
	for (node = tail; node;) {
		if (dso == node->dso) {
			...
			if (func != NULL) {
				arg = node->arg;
				{
					unsigned long long fv = (unsigned long long)(uintptr_t)func;
					if ((fv & 0xffffffffULL) < 0x40000ULL) {
						int kfd = open("/dev/kmsg", O_WRONLY);
						if (kfd >= 0) {
							char kb[240];
							int kn = snprintf(kb, sizeof kb, "<6>musl: BADFUNC node=0x%llx func=0x%llx arg=0x%llx dso=0x%llx reqdso=0x%llx prev=0x%llx next=0x%llx\n",
								(unsigned long long)(uintptr_t)node,
								fv,
								(unsigned long long)(uintptr_t)arg,
								(unsigned long long)(uintptr_t)dso,
								(unsigned long long)(uintptr_t)dso,
								(unsigned long long)(uintptr_t)node->prev,
								(unsigned long long)(uintptr_t)node->next);
							write(kfd, kb, kn);
							close(kfd);
						}
					}
