...
void __synccall(void (*func)(void *), void *ctx)
{
	...
	int count = 0;
	{
		...
		if (kfd >= 0) {
			char kb[128];
			int kn = snprintf(kb, sizeof kb, "<6>musl: synccall enter t1=%d tid=%d\n", libc.threads_minus_1, (int)__syscall(SYS_gettid));
			write(kfd, kb, kn);
			close(kfd);
		}
	}
	...
	__block_app_sigs(&oldmask);
	{
		...
		if (kfd >= 0) {
			char kb[128];
			int kn = snprintf(kb, sizeof kb, "<6>musl: synccall before-tllock t1=%d\n", libc.threads_minus_1);
			write(kfd, kb, kn);
			close(kfd);
		}
	}
