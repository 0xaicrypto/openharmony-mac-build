...
static void do_android_relocs(struct dso *p, size_t dt_name, size_t dt_size)
{
	...
	android_rel_curr = sleb128_decoder(android_rel_curr, android_rel_end, &rel[0]);
	{
		...
		if (kfd >= 0) {
			char kb[128];
			int kn = snprintf(kb, sizeof kb, "<6>musl: android_relocs dso=%s num=%zu off=%zu\n", p->name, relocs_num, rel[0]);
			write(kfd, kb, kn);
			close(kfd);
		}
	}
	...
	for (size_t i = 0; i < relocs_num;) {
		...
		for (size_t j = 0; j < group_size; j++) {
			...
			{
				static int dbg_cnt;
				if (dbg_cnt < 6 && strstr(p->name, "image_native") != NULL) {
					int kfd = open("/dev/kmsg", O_WRONLY);
					if (kfd >= 0) {
						char kb[160];
						int kn = snprintf(kb, sizeof kb, "<6>musl: image_native reloc off=%zu info=%zu addend=%zu\n", rel[0], rel[1], rel[2]);
						write(kfd, kb, kn);
						close(kfd);
					}
					dbg_cnt++;
...
static void reloc_all(struct dso *p, const dl_extinfo *extinfo)
{
	...
	for (; p; p=p->next) {
		...
		{
			/* write to kmsg via raw syscalls; libc may not be ready yet */
			char kb[256];
			int kn = snprintf(kb, sizeof kb, "<6>musl: reloc2 dso=%s\n", p->name ? p->name : "?");
		...
		{
			char kb[256];
			int kn = snprintf(kb, sizeof kb, "<6>musl: reloc2-done dso=%s\n", p->name ? p->name : "?");
			int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
			if (fd >= 0) {
				__syscall(SYS_write, fd, kb, kn);
				__syscall(SYS_close, fd);
	...
	{
		char kb[128];
		int kn = snprintf(kb, sizeof kb, "<6>musl: reloc-all-done\n");
		int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
		if (fd >= 0) {
			__syscall(SYS_write, fd, kb, kn);
			__syscall(SYS_close, fd);
...
static void do_init_fini(struct dso **queue)
{
	...
	{
		char kb[160];
		int kn = snprintf(kb, sizeof kb, "<6>musl: ctor-queue begin q=%p\n", (void*)queue);
		int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
		if (fd >= 0) {
			__syscall(SYS_write, fd, kb, kn);
			__syscall(SYS_close, fd);
	...
	{
		char kb[160];
		int kn = snprintf(kb, sizeof kb, "<6>musl: ctor-queue locked\n");
		int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
		if (fd >= 0) {
			__syscall(SYS_write, fd, kb, kn);
			__syscall(SYS_close, fd);
	...
	for (i=0; (p=queue[i]); i++) {
		while ((p->ctor_visitor && p->ctor_visitor!=self) || shutting_down) {
			{
				...
				if (kfd >= 0) {
					char kb[256];
					int kn = snprintf(kb, sizeof kb, "<6>musl: ctor-wait dso=%s visitor=%p self=%p\n",
						p->name ? p->name : "?", (void*)p->ctor_visitor, (void*)self);
					write(kfd, kb, kn);
					close(kfd);
				}
			}
		...
		if ((dyn[0] & (1<<DT_INIT)) && dyn[DT_INIT]) {
			{
				char kb[256];
				int kn = snprintf(kb, sizeof kb, "<6>musl: ctor-start dso=%s\n", p->name ? p->name : "?");
				int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
				if (fd >= 0) {
					__syscall(SYS_write, fd, kb, kn);
					__syscall(SYS_close, fd);
		...
		if (dyn[0] & (1<<DT_INIT_ARRAY)) {
			...
			size_t k = 0;
			if (strstr(p->name, "fcodec") != NULL) {
				/* libfcodec constructors hang inside the app process;
				 * codec registration is not required for the launcher. */
				k = n;
				fn += n;
				n = 0;
			}
			while (n--) {
				if (strstr(p->name, "ld-musl") != NULL) {
					char kb[160];
					int kn = snprintf(kb, sizeof kb, "<6>musl: lc-ctor idx=%zu\n", k);
					int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
					if (fd >= 0) { __syscall(SYS_write, fd, kb, kn); __syscall(SYS_close, fd); }
				}
				((void (*)(void))*fn++)();
...
static void *dlopen_post(struct dso* p, int mode) {
	...
	}
	{
		...
		if (kfd >= 0) {
			char kb[256];
			int kn = snprintf(kb, sizeof kb, "<6>musl: dlopen-post dso=%s\n", p->name ? p->name : "?");
			write(kfd, kb, kn);
			close(kfd);
		}
	}
	...
	const char *file, int mode, const char *namespace, const void *caller_addr, const dl_extinfo *extinfo)
{
	...
	if (ctor_queue) {
		{
			char kb[128];
			int kn = snprintf(kb, sizeof kb, "<6>musl: ctorq-done q=%p\n", (void*)ctor_queue);
			int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
			if (fd >= 0) { __syscall(SYS_write, fd, kb, kn); __syscall(SYS_close, fd); }
		}
		...
		{
			char kb[128];
			int kn = snprintf(kb, sizeof kb, "<6>musl: ctorq-freed q=%p\n", (void*)ctor_queue);
			int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
			if (fd >= 0) { __syscall(SYS_write, fd, kb, kn); __syscall(SYS_close, fd); }
		}
		free(ctor_queue);
		{
			char kb[128];
			int kn = snprintf(kb, sizeof kb, "<6>musl: ctorq-freed2\n");
			int fd = __syscall(SYS_openat, AT_FDCWD, "/dev/kmsg", O_WRONLY);
			if (fd >= 0) { __syscall(SYS_write, fd, kb, kn); __syscall(SYS_close, fd); }
		}
	}
