...
cmd_cc = $(CC.$(TOOLSET)) -c -o $@ $< $(GYP_CFLAGS) $(DEPFLAGS) $(CFLAGS.$(TOOLSET))

quiet_cmd_cxx = CXX($(TOOLSET)) $@
cmd_cxx = $(CXX.$(TOOLSET)) -c -o $@ $< $(GYP_CXXFLAGS) $(DEPFLAGS) $(CXXFLAGS.$(TOOLSET))
%(extra_commands)s
...
if sys.platform == 'darwin':
    # darwin port: mac 链接器不认 --start-group / thin archive
    LINK_COMMANDS_LINUX = LINK_COMMANDS_LINUX.replace(
        'quiet_cmd_link = LINK($(TOOLSET)) $@',
        'HOST_LINK_FLAGS_host = $(LD_INPUTS) $(filter-out -ldl -lrt,$(LIBS)) -Wl,-all_load\nHOST_LINK_FLAGS_target = -Wl,--start-group $(LD_INPUTS) -Wl,--end-group $(LIBS)\nquiet_cmd_link = LINK($(TOOLSET)) $@')
    LINK_COMMANDS_LINUX = LINK_COMMANDS_LINUX.replace(
        'cmd_link = $(LINK.$(TOOLSET)) $(GYP_LDFLAGS) $(LDFLAGS.$(TOOLSET)) -o $@ -Wl,--start-group $(LD_INPUTS) $(LIBS) -Wl,--end-group',
        'cmd_link = $(LINK.$(TOOLSET)) $(GYP_LDFLAGS) $(LDFLAGS.$(TOOLSET)) -o $@ $(HOST_LINK_FLAGS_$(TOOLSET))')
    LINK_COMMANDS_LINUX = LINK_COMMANDS_LINUX.replace(
        'cmd_link_host = $(LINK.$(TOOLSET)) $(GYP_LDFLAGS) $(LDFLAGS.$(TOOLSET)) -o $@ -Wl,--start-group $(LD_INPUTS) -Wl,--end-group $(LIBS)',
        'cmd_link_host = $(LINK.$(TOOLSET)) $(GYP_LDFLAGS) $(LDFLAGS.$(TOOLSET)) -o $@ $(LD_INPUTS) $(filter-out -ldl -lrt,$(LIBS))')
    LINK_COMMANDS_LINUX = LINK_COMMANDS_LINUX.replace(
        '        $(AR.$(TOOLSET)) crsT $(1) @$(1).$(OBJ_FILE_LIST)',
        '        $(AR.$(TOOLSET)) crs $(1) @$(1).$(OBJ_FILE_LIST)')
    LINK_COMMANDS_LINUX = LINK_COMMANDS_LINUX.replace(
