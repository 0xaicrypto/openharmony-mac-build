...
OUT=obj/out/arm64_virt/obj/third_party/musl/intermidiates/linux/musl_src_ported/src/thread/soft_musl_src_shared/__wait.o
SRC=obj/third_party/musl/intermidiates/linux/musl_src_ported/src/thread/__wait.c
/opt/homebrew/bin/ccache $CLANG -MMD -MF "$OUT.d" $DEFINES $INCLUDE_DIRS $CFLAGS $CFLAGS_C -c "$SRC" -o "$OUT"
echo "__wait.o compiled"
