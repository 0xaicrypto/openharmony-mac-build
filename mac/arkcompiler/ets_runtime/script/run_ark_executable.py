...
def collect_darwin_dylib_dirs(script_file: str) -> str:
    """darwin port: 收集整个 host 工具链目录树下的所有 dylib 目录"""
    dirs = set()
    base = os.path.dirname(os.path.abspath(script_file))
    # 向上找到工具链根 (clang_arm64 / clang_x64 / toolchain)
    while True:
        name = os.path.basename(base)
        if name.startswith("clang_") or name == "arm64_virt" or name == "obj":
            break
    ...
    for root, _, files in os.walk(base):
        if any(f.endswith('.dylib') for f in files):
            dirs.add(root)
...
def process_open(args: object) -> [str, object]:
    ...
    if args.env_path:
        # darwin port: 补充所有 dylib 目录 (icu/hilog 等)
        if sys.platform == "darwin":
            extra = collect_darwin_dylib_dirs(args.script_file)
            if extra:
                args.env_path = str(args.env_path) + ":" + extra
        # use the given env-path
        cmd = args.script_file
        cmd += " {}".format(args.script_options) if args.script_options else ""
        cmd += " {}".format(args.script_args) if args.script_args else ""
        # process for running executable directly
        # darwin port: mac 用 DYLD_LIBRARY_PATH
        _env = {'LD_LIBRARY_PATH': str(args.env_path)}
        if sys.platform == "darwin":
            _env['DYLD_LIBRARY_PATH'] = str(args.env_path)
        # darwin port: shell=True 会丢弃 DYLD_*, 直接 exec
        ...
        if args.qemu_binary_path:
            ...
            # process for running executable directly
            _env = {'LD_LIBRARY_PATH': str(env_path)}
            if sys.platform == "darwin":
                _env['DYLD_LIBRARY_PATH'] = str(env_path)
