...
{
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ const char *m="<6>composer-dbg: LoadVdiSo enter\n"; write(kfd,m,strlen(m)); close(kfd);} }
...
#endif // COMPOSER_VDI_DEFAULT_LIBRARY_ENABLE
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>composer-dbg: LoadVdiSo done handle=%p\n",(void*)libHandle_); write(kfd,kb,kn); close(kfd);} }
