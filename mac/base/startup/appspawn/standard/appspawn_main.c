...
static void CheckPreload(char *const argv[])
{
    ...
    buf[nread] = 0;
    asdbg("ASDBG: execv path=%s argv0=%s\n", buf, argv[0]);
    ret = execv(buf, argv);
    asdbg("ASDBG: execv FAILED err=%d\n", errno);
...
int main(int argc, char *const argv[])
{
    ...
    InitCommonEnv();
    asdbg("ASDBG: before CheckPreload, preload=%s\n", getenv("LD_PRELOAD") ? getenv("LD_PRELOAD") : "(null)");
    CheckPreload(argv);
    asdbg("ASDBG: after CheckPreload\n");
...
#endif
    arg = &argTemp->arg;
    asdbg("ASDBG: after GetAppSpawnStartArg, arg=%p initArg=%d\n", (void*)arg, arg->initArg);
    if (arg->initArg == 0) {
    ...
    asdbg("ASDBG: before StartSpawnService\n");
    AppSpawnContent *content = StartSpawnService(arg, argvSize, argc, argv);
    asdbg("ASDBG: StartSpawnService returned %p\n", (void*)content);
    if (content == NULL) { asdbg("ASDBG: content NULL, will return 0\n"); }
    if (content != NULL) {
