...
int AppSpawnLoadAutoRunModules(int type)
{
    ...
    if (g_moduleMgr[type].moduleMgr != NULL) {
        asdbg("AppSpawnLoadAutoRunModules type=%d name=%s already loaded %p", type, g_moduleMgr[type].moduleName, (void*)g_moduleMgr[type].moduleMgr);
        return 0;
    }
    if (type == MODULE_APPSPAWN) {
        const char *testlibs[] = {
            "/system/lib64/libcolor_manager.z.so",
            "/system/lib64/libimage_utils.z.so",
            "/system/lib64/libpluginmanager.z.so",
            "/system/lib64/libskia_canvaskit.z.so",
            "/system/lib64/libimage_native.z.so",
            "/system/lib64/libability_manager.z.so",
            NULL
        };
        for (int i = 0; testlibs[i] != NULL; i++) {
            asdbg("TESTDL: %s", testlibs[i]);
            void *th = dlopen(testlibs[i], RTLD_LAZY | RTLD_GLOBAL);
            const char *te = th ? NULL : dlerror();
            asdbg("TESTDL done: %s -> %p err=%s", testlibs[i], th, te ? te : (th ? "ok" : "noerr"));
            if (th != NULL) {
                dlclose(th);
            }
        }
        ...
        const char *deps[] = {
            ...
            "/system/lib64/libability_manager.z.so",
            "/system/lib64/libapp_manager.z.so",
            "/system/lib64/libchild_process_manager.z.so",
            "/system/lib64/libability_deps_wrapper.z.so",
            "/system/lib64/libruntime.z.so",
            "/system/lib64/libability_thread.z.so",
            "/system/lib64/libabilitykit_native.z.so",
            "/system/lib64/libuiabilitykit_native.z.so",
            "/system/lib64/libapp_context.z.so",
            "/system/lib64/libapp_context_utils.z.so",
            "/system/lib64/libfreeze_util.z.so",
            "/system/lib64/libhmicuuc.z.so",
            "/system/lib64/libbase.z.so",
            "/system/lib64/libextractresourcemanager.z.so",
            "/system/lib64/libstring_utils.z.so",
            "/system/lib64/libwant.z.so",
            "/system/lib64/libappexecfwk_base.z.so",
            "/system/lib64/libappkit_native.z.so",
            NULL
        };
        for (int i = 0; deps[i] != NULL; i++) {
            asdbg("PREDLOPEN: %s", deps[i]);
            void *h = dlopen(deps[i], RTLD_LAZY | RTLD_GLOBAL);
            const char *e = h ? NULL : dlerror();
            asdbg("PREDLOPEN done: %s -> %p err=%s", deps[i], h, e ? e : (h ? "ok" : "noerr"));
            if (h != NULL) {
                dlclose(h);
            }
        }
        asdbg("PREDLOPEN ace itself");
        void *h2 = dlopen("/system/lib64/appspawn/appspawn/libappspawn_ace.z.so", RTLD_LAZY | RTLD_GLOBAL);
        const char *e2 = h2 ? NULL : dlerror();
        asdbg("PREDLOPEN ace done: %p err=%s", h2, e2 ? e2 : (h2 ? "ok" : "noerr"));
        if (h2 != NULL) {
            dlclose(h2);
        }
#endif
    ...
    asdbg("AppSpawnLoadAutoRunModules type=%d name=%s -> %p", type, g_moduleMgr[type].moduleName, (void*)g_moduleMgr[type].moduleMgr);
