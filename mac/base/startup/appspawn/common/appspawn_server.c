...
{
    asdbg("AppSpawnChild enter id=%d", client->id);
    ...
    asdbg("AppSpawnChild: ClearEnvHook");
    ...
    asdbg("AppSpawnChild: SpawningHook");
...
{
    asdbg("AppSpawnProcessMsg id=%d mode=%d sandboxNsFlags=0x%x", client->id, content->mode, content->sandboxNsFlags);
