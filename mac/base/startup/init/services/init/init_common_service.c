...
static int SetPerms(const Service *service)
{
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM keepcap %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM setgid %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    INIT_ERROR_CHECK(ServiceSetGid(service) == SERVICE_SUCCESS, return INIT_EGIDSET,
        "Service error %d %s, failed to set gid.", errno, service->name);

    // set seccomp policy before setuid
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM seccomp %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    if (service->servPerm.uID != 0) {
        { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM setuid %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM before capset %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    INIT_ERROR_CHECK(capset(&capHeader, capData) == 0, return SERVICE_FAILURE,
        "capset failed for service: %s, error: %d", service->name, errno);
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM after capset %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    for (unsigned int i = 0; i < service->servPerm.capsCnt; ++i) {
        if (service->servPerm.caps[i] == FULL_CAP) {
     ...
     */
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM before hook60 %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ServiceHookExecute(service->name, NULL, INIT_SERVICE_SET_PERMS);
#endif
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: PERM done %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    return SERVICE_SUCCESS;
}
...
static int InitServiceProperties(Service *service, const ServiceArgs *pathArgs)
{
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step sandbox %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step token %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step env %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    if (!IsOnDemandService(service)) {
        { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step socket %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step file %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step holdfds %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    // write pid
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step pid %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
    ...
    // permissions
    { int kfd=open("/dev/kmsg",O_WRONLY); if(kfd>=0){ char kb[128]; int kn=snprintf(kb,sizeof kb,"<6>init-dbg: ISP step perms %s\n", service->name); write(kfd,kb,kn); close(kfd);} }
...
{
    {
        ...
        if (kfd >= 0) {
            char kb[128];
            int kn = snprintf(kb, sizeof kb, "<6>init-dbg: RunChildProcess %s\n", service->name);
            write(kfd, kb, kn);
            close(kfd);
        }
    }
    ...
    // fail must exit sub process
    {
        ...
        if (kfd >= 0) {
            char kb[128];
            int kn = snprintf(kb, sizeof kb, "<6>init-dbg: before InitServiceProperties %s\n", service->name);
            write(kfd, kb, kn);
            close(kfd);
        }
    }
    int ret = InitServiceProperties(service, pathArgs);
    {
        ...
        if (kfd >= 0) {
            char kb[128];
            int kn = snprintf(kb, sizeof kb, "<6>init-dbg: InitServiceProperties %s ret=%d lastErrno=%d\n", service->name, ret, service->lastErrno);
            write(kfd, kb, kn);
            close(kfd);
        }
    }
