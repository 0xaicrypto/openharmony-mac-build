...
if __name__ == '__main__':
    ...
    flexlexer_candidates = [
        "/usr/include/FlexLexer.h",
        "/Library/Developer/CommandLineTools/usr/include/FlexLexer.h"
    ]
    flexlexer_src = next((p for p in flexlexer_candidates if os.path.exists(p)), None)
    if flexlexer_src is None:
        raise RuntimeError("FlexLexer.h not found in %s" % flexlexer_candidates)
    shutil.copy2(flexlexer_src, flexlexer_file)
