...
get_thread_num() {
    ...
    if [ "${cfs_quota_us}" != -1 -a "${cfs_period_us}" != 0 ]; then
        ...
        if [ -f /proc/cpuinfo ]; then
            ...
            PROCESSORS=$(sysctl -n hw.ncpu 2>/dev/null || echo 8)
            echo "cpuinfo(darwin): "$PROCESSORS
        fi
    fi
}
...
do_install () {
    cp -f ${workdir}/out/Release/libjsvm.108.dylib ${TARGET_GEN_DIR}/libjsvm.so
}

do_install_hwasan () {
    mkdir -p ${TARGET_GEN_DIR}/asan
    cp -f ${workdir}/out/Release/libjsvm.108.dylib ${TARGET_GEN_DIR}/libjsvm.so/asan
}
...
do_env() {
    ...
    if [[ "${TARGET_CPU}" = "arm" ]]; then
        ...
        cflags+=" -DV8_OS_OH=1"
        cflags+=" -mfpu=neon"
        cflags_host="-m64 -isysroot $(xcrun --show-sdk-path 2>/dev/null) -DV8_ENABLE_PRIVATE_MAPPING_FORK_OPTIMIZATION=0 -DHAVE_EXECINFO_H=1"
        # darwin port: host 编译(v8 误用 linux 平台) 只补 linux 子目录头, 避免 musl 根头
        # 抢 libc++ 的 include_next (stddef.h/stdlib.h 需来自 mac SDK)
        if [ -d "${SYSROOT}/usr/include/aarch64-linux-ohos" ]; then
            cflags_host+=" -I${NODE_PATH}/out/Release_headers"
        fi
        ARCH="aarch64"
    ...
    # host env (darwin port: 宿主编译用 mac 平台)
