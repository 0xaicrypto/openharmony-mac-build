...
void AddDirectory(const std::string& path, const std::string& outpath)
{
    ...
    // sort the readdir result (copy names, dirent pointers are single-buffer
    // reused by readdir on some platforms, e.g. macOS)
    struct Entry {
        std::string name;
        bool isDir;
        bool operator<(const Entry& o) const { return name < o.name; }
    };
    auto dirSet = std::set<Entry>();

    while ((pDirent = readdir(pDir)) != nullptr) {
        dirSet.insert({ std::string(pDirent->d_name), pDirent->d_type == DT_DIR });
    }
    ...
    for (const auto& d : dirSet) {
        if (d.isDir) {
            ...
            AddDirectory(p + d.name, op + d.name);
            continue;
        }
        AppendFile(p + d.name, op + d.name);
    }
