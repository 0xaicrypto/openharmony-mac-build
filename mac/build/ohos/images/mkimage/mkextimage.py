...
def setup_tool_paths() -> None:
    """darwin port: host 工具在 clang_arm64/thirdparty 下, 且 mac 用 DYLD_LIBRARY_PATH"""
    import sys as _sys
    if _sys.platform == "darwin":
        base = os.path.realpath("./clang_arm64/thirdparty")
        os.environ['PATH'] = os.path.join(base, "e2fsprogs") + os.pathsep + \
            os.environ.get('PATH', '')
        os.environ['DYLD_LIBRARY_PATH'] = os.path.join(base, "selinux") + ":" + \
            os.path.join(base, "pcre2")
...
def build_run_e2fsdroid(args) -> int:
    ...
    # darwin port: mac 上 host 工具在 clang_arm64/thirdparty 下, 用 DYLD_LIBRARY_PATH
    import sys as _sys
    if _sys.platform == "darwin":
        base = os.path.realpath("./clang_arm64/thirdparty")
        os.environ['PATH'] = os.path.join(base, "e2fsprogs") + os.pathsep + \
