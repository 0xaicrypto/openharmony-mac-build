...
def get_needed_lib(file_path: str) -> list:
    # darwin port: mac 无 readelf, 用 llvm-readelf
    import sys as _sys
    _readelf = "readelf"
    if _sys.platform == "darwin":
        _candidates = [
            "prebuilts/clang/ohos/darwin-arm64/llvm/bin/llvm-readelf",
            "prebuilts/clang/ohos/darwin-x86_64/llvm/bin/llvm-readelf",
        ]
        for _c in _candidates:
            _p = os.path.join("/Users/hui/ohos-src", _c)
            if os.path.exists(_p):
                _readelf = _p
                break
    cmd = " ".join([_readelf, "-d", file_path])
    res = run_cmd(cmd)
    if res[1] != 0:
