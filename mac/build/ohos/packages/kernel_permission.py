...
class KernelPermission():
    ...
    def regist_llvm_objcopy_path(root_path):
        ...
        """
        # darwin port: 优先按 host 平台查找
        import sys as _sys
        if _sys.platform == "darwin":
            _candidates = [
                "prebuilts/clang/ohos/darwin-arm64/llvm/bin/llvm-objcopy",
                "prebuilts/clang/ohos/darwin-x86_64/llvm/bin/llvm-objcopy",
            ]
        else:
            _candidates = ["prebuilts/clang/ohos/linux-x86_64/llvm/bin/llvm-objcopy"]
        llvm_objcopy_path = ""
        for _c in _candidates:
            _p = os.path.join(root_path, _c)
            if os.path.exists(_p):
                llvm_objcopy_path = _p
                break
